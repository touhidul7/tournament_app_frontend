...
const BrMatchJoin = () => {
    const { balance, CheckUser, sendNotification } = useOutletContext();
    const { id } = useParams();
    const navigate = useNavigate();
    const [isJoining, setIsJoining] = useState(false); // prevent spamming
    ...
    const onSubmit = (data) => {
        ...
        if (isJoining) return; // prevent double-click spam
        setIsJoining(true);
        ...
        request
            .then((response) => {
                if (response.status === 201) {
                    ...
                    sendNotification(
                        user.user.uid,
                        `You have successfully joined the match ${matchDetails.match_name} with entry fee ${fee} BDT.`
                    );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='max-w-md mx-auto h-screen font-Jakarta bg-mainbg'>
            ...
            <div className="w-full  flex items-center justify-center mt-4">
                <button
                    type="submit"
                    disabled={isJoining}
                    className={`w-[95%] p-3 mb-20 rounded text-lg font-semibold text-white ${isJoining ? "bg-gray-400 cursor-not-allowed" : "bg-green-500"
                        }`}
                >
                    {isJoining ? "Joining..." : "Pay & Join Now"}
                </button>
