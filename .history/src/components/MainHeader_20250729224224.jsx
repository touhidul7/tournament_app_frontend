...
const MainHeader = ({ balance, settings }) => {
    console.log(settings);

    const [notifications, setNotifications] = useState([]);
    const [seenCount, setSeenCount] = useState(0); // Count of notifications user has seen
        ...
        <div>
            <div className='py-2 flex items-center justify-between border-b border-cardbg bg-cardbg'>
                <NavLink to={`/`} className='flex items-center gap-2'>
                    <h2 className='text-white font-medium text-lg ml-2'>App Name</h2>
                </NavLink>
                <div className='flex items-center justify-end gap-0'>
                    <button onClick={NotificationClick} className='relative px-2 py-2 ml-9'>
